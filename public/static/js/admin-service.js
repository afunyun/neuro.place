...
class AdminService {
     ...
     * @private
     */
    async _makeAdminCheckRequest(userId, token, cacheKey) {
        let lastError;
        ...
        for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
            try {
                ...
                if (response.ok) {
                    ...
                    // Authentication/authorization failures shouldn't be retried
                    this.cache.set(cacheKey, {
                        isAdmin: false,
                        timestamp: Date.now()
                    });
                ...
                if (error.name === 'AbortError') {
                    ...
                    console.warn(`Admin check failed on attempt ${attempt}:`, error.message);
                }
        ...
        // Cache negative result to prevent repeated failed requests
        this.cache.set(cacheKey, {
            isAdmin: false,
            timestamp: Date.now()
        });
    ...
    async bulkCheckAdmins(users) {
        ...
        // For now, do individual requests (could be optimized with batch API)
...
if (typeof window !== 'undefined') {
    ...
    // Preload current user admin status when DOM is ready
